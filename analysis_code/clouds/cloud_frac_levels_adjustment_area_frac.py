...
pert_file = 'cloud_frac_02261_' + pert_suite + '_ap5_2014-2048.pp'
############################################
...
for i in range(1):
    ...
    plt.colorbar(mesh, shrink = 0.9, label = 'Cloud area fraction adjustment', \
    ...
    plt.title(title + ' ' + height_label_list[i] + ' cloud area frac adjustment', \
               fontsize = 'medium')
    plt.savefig(save_plot_target + height_label_list[i] + '_cloud_area_frac_adjustment', dpi = 300)
    plt.show()
