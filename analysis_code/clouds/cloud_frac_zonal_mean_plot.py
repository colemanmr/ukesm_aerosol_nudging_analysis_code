...
"""
Script to plot cross section of time zonal mean cloud fraction from a single
suite
"""
...
# load cube - CAN CHANGE
suite = 'br793'
file = 'cloud_frac_annual_zonal_mean_' + suite + '.nc'
cube = iris.load_cube(diag_dir + file,\
          'annual_zonal_mean_cloud_volume_fraction_in_atmosphere_layer')


# Time mean cube
cloud, cloud_stdev, cloud_double_se =\
flux_mod.time_mean_cube(cube)


# Change units of level height to km
cloud.coord('level_height').convert_units('km')
...
target = plot_dir + 'cloud_frac_zonal_time_mean_plot_' + suite


# Plot zonal mean cross section
plt.figure()
mesh = iplt.pcolormesh(cloud[0:63],
