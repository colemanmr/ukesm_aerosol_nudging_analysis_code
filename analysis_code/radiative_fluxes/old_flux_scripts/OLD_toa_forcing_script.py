...
def main(cont_filename, pert_filename, sky_type):
    ...
    flux_mod.toa_flux(control_filepath, sky_type = sky_type,\
                      spinup_years = 1)   ########################NEED TO PUT SPINUP IN FUNCTION CALL#########
    #!! ALso should change spinup to be years beginning adn ending so can use if have extra years afterwards i'm not using 
    ...
    flux_mod.toa_flux(perturbed_filepath, sky_type = sky_type,\
                      spinup_years = 1)   ########################NEED TO PUT IN FUNCTION CALL#########
    ...
    #!! Change so only uses the years after spinup! !!#
...
#    years = np.linspace(1, no_years, no_years)
    ...
    #     fontsize = 'medium')
    plt.plot(model_years, area_mean_net_forcing.data, color = 'black', linestyle = '-', label = 'net')  
    plt.plot(model_years, area_mean_sw_forcing.data, color = 'mediumblue', linestyle = '-', label = 'sw')
    plt.plot(model_years, area_mean_lw_forcing.data, color = 'r', linestyle = '-', label = 'lw')
