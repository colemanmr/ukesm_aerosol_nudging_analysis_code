...
def main():
    ...
    plt.title(u'PI SO$_{2}$ net TOA radiative flux')
    plt.xlabel('Model year')
    plt.ylabel(u'radiative flux / Wm$^{-2}$')
    plt.savefig(plot_directory + 'so2_free_net_toa_flux_time_series', \
                dpi=220)
    plt.show()
    
    fig, axarr = plt.subplots(3, sharex = True)
    axarr[0].set_title(u'PI SO$_{2}$ TOA downwards fluxes / Wm$^{-2}$')
...
#    axarr[2].set_ylim(-240.0,-239.5)
    plt.savefig(plot_directory + 'so2_free_net_toa_fluxes_time_series', \
                dpi = 220)
    plt.show()
...
#    plt.title('Net downwards radiative flux year 11 - year 1')
