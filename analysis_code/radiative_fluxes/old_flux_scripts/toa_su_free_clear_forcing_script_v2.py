...
def main():
    ...
    def control():
        ...
        filepath = '/storage/silver/scenario/nn819853/diags/clear_and_clean_net/free_cont_ap5.pp'
    ...
    def su_free():
        ...
        # set directory and filename of diagnostics - should be for one year
        filepath = '/storage/silver/scenario/nn819853/diags/clear_and_clean_net/free_su_ap5.pp'
    ...
    plt.title(u'PI SU free running clear-sky TOA forcing')
    plt.xlabel('Model year')
    plt.ylabel(u'radiative flux / Wm$^{-2}$')
    plt.savefig(plot_directory + 'so2_free_toa_total_forcing_clear_time_series', \
                dpi=220)
    plt.show()
    
    fig, axarr = plt.subplots(3, sharex = True)
    axarr[0].set_title(u'PI SU clear-sky TOA forcings / Wm$^{-2}$')
...
#    axarr[2].set_ylim(-240.0,-239.5)
    plt.savefig(plot_directory + 'so2_free_toa_forcing_clear_time_series', \
                dpi = 220)
    plt.show()   
    ...
    plt.title(u'Control vs SU free clear-sky TOA net flux')
    plt.xlabel('Model year')
    plt.ylabel(u'radiative flux / Wm$^{-2}$')
    plt.savefig(plot_directory + 'so2_control_free_toa_mean_net_flux_clear_time_series', \
                dpi=220)
    plt.show()
    
    fig, axarr = plt.subplots(3, sharex = True)
    axarr[0].set_title(u'Free control vs SU downwards clear-sky TOA fluxes / Wm$^{-2}$', \
         fontsize = 'medium')
...
#    axarr[2].set_ylim(-242,-239)
    ...
    plt.tight_layout()
    plt.savefig(plot_directory + 'so2_control_free_toa_mean_net_fluxes_clear_time_series', \
                dpi = 400)
    plt.show()      
    ...
    plt.title('Free control - perturbed SU mean clear-sky TOA forcing', fontsize = 'medium')
#    plt.tight_layout()
    plt.savefig(plot_directory + 'su_free_mean_toa_forcing_clear_map.png', \
                dpi = 220)
    plt.show()
    ...
    current_map = plt.gca()
    current_map.coastlines(linewidth = 1)
    plt.title('Free control - perturbed SU mean clear-sky TOA SW forcing')
    plt.savefig(plot_directory + 'su_free_mean_toa_shortwave_forcing_clear_map.png', \
                dpi = 220)
    plt.show()
    ...
    current_map = plt.gca()
    current_map.coastlines(linewidth = 1)
#    plt.title('Free control - perturbed SU mean clear-sky TOA LW forcing')
    plt.title('Sulphate longwave cloud-free forcing', fontsize = 'medium')
    plt.savefig(plot_directory + 'su_free_mean_toa_longwave_forcing_clear_map.png',\
                dpi = 400)
    plt.show()

    plt.figure()
    mesh = iplt.pcolormesh(mean_net_forcing, cmap = 'bwr', vmin = -26, vmax = 26)
    plt.colorbar(mesh, shrink = 0.90, label = u' W m$^{-2}$', orientation = 'horizontal')
    current_map = plt.gca()
    current_map.coastlines(linewidth = 1)
    plt.title('Free control - perturbed SU mean clear-sky TOA forcing', fontsize = 'medium')
#    plt.tight_layout()
    plt.savefig(plot_directory + 'su_free_mean_toa_forcing_clear_map_NEW.png', \
                dpi = 220)
    plt.show()
