...
def main():
    ...
    filepath = '/storage/silver/scenario/nn819853/diags/clear_and_clean_net/ap5.pp'
    filepath2 = '/storage/silver/scenario/nn819853/diags/clear_and_clean_net/ap4.pp'
    
    # read diagnostics into cubes
    sw_down = iris.load(filepath, 'toa_incoming_shortwave_flux')
    lw_up_levels = iris.load(filepath2, 'm01s02i517')
    sw_up_levels = iris.load(filepath2, 'm01s01i517')
    
#    print(iris.load(filepath2))
    
    lw_up = lw_up_levels[85]
    sw_up = sw_up_levels[85]
    
    # make a normal list of cubes - don't think is same as cubelist
    cubes = [sw_down, lw_up, sw_up]
    print(sw_down, lw_up, sw_up)
    ...
    plt.title('PD control net clean sky TOA radiative flux')
    plt.xlabel('Model year')
    plt.ylabel(u'radiative flux / Wm$^{-2}$')
    plt.savefig(plot_directory + 'PD_control_net_toa_clean_flux_time_series', \
                dpi=220)
    plt.show()
    
    fig, axarr = plt.subplots(3, sharex = True)
    axarr[0].set_title(u'TOA clean sky downwards fluxes / Wm$^{-2}$')
...
#                  str(year + 1))
#        plt.savefig(plot_directory + 'net_flux_clean_sky_toa' + str(year+1) + '.png', \
#                    dpi = 220)
#        plt.show()
...
#    plt.title('Net downwards clean sky radiative flux year 11 - year 1')
#    plt.savefig(plot_directory + 'net_flux_clean_sky_toa_11-1.png', dpi = 220)
#    plt.show()
