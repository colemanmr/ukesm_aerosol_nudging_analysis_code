...
def main():
    ...
    plt.title('Free SU net TOA clear-sky flux')
    plt.xlabel('Model year')
    plt.ylabel(u'radiative flux / Wm$^{-2}$')
    plt.savefig(plot_directory + 'free_su_net_toa_clear_flux_time_series', \
                dpi=220)
    plt.show()
    
    fig, axarr = plt.subplots(3, sharex = True)
    axarr[0].set_title(u'Free SU net TOA clear-sky fluxes / Wm$^{-2}$')
...
#    axarr[2].set_ylim(-239.9,-239.5)
    plt.savefig(plot_directory + 'free_su_net_toa_clear_fluxes_time_series', \
                dpi = 220)
    plt.show()
