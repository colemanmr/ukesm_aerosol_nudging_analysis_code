...
def main():
    """
    Script for determining TOA annual net radiative fluxes from UM 
    diagnostics for radaiative fluxes at TOA (01207,01208,02205) 
    and plotting as lat-lon map and determining a grid weighted average.
    Also plots the annual time series of mean net flux.
    """
...
#    print(net_down.coord('time').bounds[-1,1])
#    print(net_down.coord('time').bounds[0,0])
    
    # initialise lists for holding net down spatial mean fluxes
    ...
    plt.title('PD control net TOA radiative flux')
    plt.xlabel('Model year')
    plt.ylabel(u'radiative flux / W m$^{-2}$')
    plt.savefig(plot_directory + 'cont_free_net_toa_flux_time_series', \
                dpi=220)
    plt.show()
...
#    axarr[2].set_ylim(-240.0,-239.5)
    plt.savefig(plot_directory + 'cont_free_net_toa_fluxes_time_series', \
                dpi = 220)
    plt.show()
...
#                  str(area_mean_float[year]) + u' W m$^{-2}$ | Year ' + \
#                  str(year + 1))
#        plt.savefig(plot_directory + 'net_flux_toa' + str(year+1) + '.png', \
#                    dpi = 220)
#        plt.show()
    ...
    plt.figure()
    mesh = iplt.pcolormesh(diff)
    plt.colorbar(mesh, shrink = 0.65, label = u' W m$^{-2}$')
    plt.title('Net downwards radiative flux year 30 - year 1')
    plt.savefig(plot_directory + 'net_flux_toa_11-1.png', dpi = 220)
    plt.show()
