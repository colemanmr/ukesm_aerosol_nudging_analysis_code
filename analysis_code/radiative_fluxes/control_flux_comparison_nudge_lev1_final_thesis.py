...
#        sky_type = file[index2 + 7 : index1 - 5]
        ...
        flux_error[j, :, 0] = fluxes[0, :, 0] - fluxes[j, :, 0]
        flux_error[j, :, 1] = fluxes[0, :, 1] + fluxes[j, :, 1]
        # does free - nudged control unlike original
 ...
 # ax.bar(label_points - 18*width/2, flux_error[0,:,0], width, yerr = flux_error[0,:,1], color = 'black', label = 'free, n=34')
    ...
    ax.bar(label_points - 15*width/2, flux_error[1,:,0], width, yerr = flux_error[1,:,1], color = 'brown', label = 'uv' + char.theta() + ', G=1/24, bl=10, r=4')
    ax.bar(label_points - 13*width/2, flux_error[2,:,0], width, yerr = flux_error[2,:,1], color = 'firebrick', label = 'uv' + char.theta() + ', G=1/12, bl=10, r=4')
    ax.bar(label_points - 11*width/2, flux_error[3,:,0], width, yerr = flux_error[3,:,1], color = 'red', label = 'uv' + char.theta() + ', G=1/6, bl=10, r=4')
    ax.bar(label_points - 9*width/2, flux_error[4,:,0], width, yerr = flux_error[4,:,1], color = 'tomato', label = 'uv' + char.theta() + ', G=1/3, bl=10, r=4')
    ax.bar(label_points - 7*width/2, flux_error[5,:,0], width, yerr = flux_error[5,:,1], color = 'lightcoral', label = 'uv' + char.theta() + ', G=1/1, bl=10, r=4')
    
    ax.bar(label_points - 4*width/2, flux_error[6,:,0], width, yerr = flux_error[6,:,1], color = 'peru', label = 'uv' + char.theta() + ', G=1/6, bl=1, r=1')
    ax.bar(label_points - 2*width/2, flux_error[7,:,0], width, yerr = flux_error[7,:,1], color = 'darkorange', label = 'uv' + char.theta() + ', G=1/6, bl=1, r=4')
    
    ax.bar(label_points + 1*width/2, flux_error[8,:,0], width, yerr = flux_error[8,:,1], color = 'green', label = 'uv' + char.theta() + ', G=1/6, bl=1, r=0')
    
    ax.bar(label_points + 4*width/2, flux_error[9,:,0], width, yerr = flux_error[9,:,1], color = 'midnightblue', label = 'uv, G=1/24, bl=10, r=4')
    ax.bar(label_points + 6*width/2, flux_error[10,:,0], width, yerr = flux_error[10,:,1], color = 'mediumblue', label = 'uv, G=1/12, bl=10, r=4')
    ax.bar(label_points + 8*width/2, flux_error[11,:,0], width, yerr = flux_error[11,:,1], color = 'royalblue', label = 'uv, G=1/6, bl=10, r=4')
    ax.bar(label_points + 10*width/2, flux_error[12,:,0], width, yerr = flux_error[12,:,1], color = 'cornflowerblue', label = 'uv, G=1/3, bl=10, r=4')
    ax.bar(label_points + 12*width/2, flux_error[13,:,0], width, yerr = flux_error[13,:,1], color = 'lightsteelblue', label = 'uv, G=1/1, bl=10, r=4')
    
    ax.bar(label_points + 15*width/2, flux_error[14,:,0], width, yerr = flux_error[14,:,1], color = 'indigo', label = 'uv, G=1/6, bl=1, r=1')
    ax.bar(label_points + 17*width/2, flux_error[15,:,0], width, yerr = flux_error[15,:,1], color = 'darkviolet', label = 'uv, G=1/6, bl=1, r=4')
    ...
    ax.text(1.2,-7.5, sky+' sky', ha = 'left', fontsize = 'large')
    ax.axhline(y=0, xmin = 0, xmax = 1, color = 'darkgrey', linestyle = ':')
    ax.axvline(x=0.53, ymin=-8, ymax=1, color='darkgrey', linestyle='--')
    ax.axvline(x=1.53, ymin=-8, ymax=1, color='darkgrey', linestyle='--')
    ax.set_ylabel(u'TOA flux control error / W m$^{-2}$')
    plt.tight_layout()
    plt.savefig(plot_dir + 'control_flux_error_not_reversed_comparison_varying_G_' + sky + '_sky_nudge_lev1',\
