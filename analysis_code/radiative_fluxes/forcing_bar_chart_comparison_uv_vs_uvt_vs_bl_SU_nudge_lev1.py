...
for i, sky in enumerate(sky_type):
    ...
    filenames = ['calc_forcing_br793_control_nudging_free_flux_'+sky+'_sky_minus_bv046_su_nudging_free_flux_'+sky+'_sky_1yr_spinup.nc',
                 ...
                 'calc_forcing_cj765_control_nudging_uvt_flux_'+sky+'_sky_minus_cj766_su_nudging_uvt_flux_'+sky+'_sky_1yr_spinup.nc',
...
#        sky_type = file[index2 + 7 : index1 - 5]
    ...
    ax.bar(label_points - 18*width/2, forcings[0,:,0], width, yerr = forcings[0,:,1], color = 'black', label = 'free')
    
    ax.bar(label_points - 15*width/2, forcings[1,:,0], width, yerr = forcings[1,:,1], color = 'brown', label = 'uv' + char.theta() + ', G=1/24, bl=10, r=4')
    ax.bar(label_points - 13*width/2, forcings[2,:,0], width, yerr = forcings[2,:,1], color = 'firebrick', label = 'uv' + char.theta() + ', G=1/12, bl=10, r=4')
    ax.bar(label_points - 11*width/2, forcings[3,:,0], width, yerr = forcings[3,:,1], color = 'red', label = 'uv' + char.theta() + ', G=1/6, bl=10, r=4')
    ax.bar(label_points - 9*width/2, forcings[4,:,0], width, yerr = forcings[4,:,1], color = 'tomato', label = 'uv' + char.theta() + ', G=1/3, bl=10, r=4')
    ax.bar(label_points - 7*width/2, forcings[5,:,0], width, yerr = forcings[5,:,1], color = 'lightcoral', label = 'uv' + char.theta() + ', G=1/1, bl=10, r=4')
    
    ax.bar(label_points - 4*width/2, forcings[6,:,0], width, yerr = forcings[6,:,1], color = 'peru', label = 'uv' + char.theta() + ', G=1/6, bl=1, r=1')
    ax.bar(label_points - 2*width/2, forcings[7,:,0], width, yerr = forcings[7,:,1], color = 'darkorange', label = 'uv' + char.theta() + ', G=1/6, bl=1, r=4')
    
    ax.bar(label_points + 1*width/2, forcings[8,:,0], width, yerr = forcings[8,:,1], color = 'green', label = 'uv' + char.theta() + ', G=1/6, bl=1, r=0')
    
    ax.bar(label_points + 4*width/2, forcings[9,:,0], width, yerr = forcings[9,:,1], color = 'midnightblue', label = 'uv, G=1/24, bl=10, r=4')
    ax.bar(label_points + 6*width/2, forcings[10,:,0], width, yerr = forcings[10,:,1], color = 'mediumblue', label = 'uv, G=1/12, bl=10, r=4')
    ax.bar(label_points + 8*width/2, forcings[11,:,0], width, yerr = forcings[11,:,1], color = 'royalblue', label = 'uv, G=1/6, bl=10, r=4')
    ax.bar(label_points + 10*width/2, forcings[12,:,0], width, yerr = forcings[12,:,1], color = 'cornflowerblue', label = 'uv, G=1/3, bl=10, r=4')
    ax.bar(label_points + 12*width/2, forcings[13,:,0], width, yerr = forcings[13,:,1], color = 'lightsteelblue', label = 'uv, G=1/1, bl=10, r=4')
    
    ax.bar(label_points + 15*width/2, forcings[14,:,0], width, yerr = forcings[14,:,1], color = 'indigo', label = 'uv, G=1/6, bl=1, r=1')
    ax.bar(label_points + 17*width/2, forcings[15,:,0], width, yerr = forcings[15,:,1], color = 'darkviolet', label = 'uv, G=1/6, bl=1, r=4')
    ...
    if sky=='cre':
        ax.text(2,-1.8, 'CRE', ha = 'center', fontsize = 'large')    
    elif sky=='dre':
        ax.text(2,-1.8, 'DRE', ha = 'center', fontsize = 'large')  
    else:
        ax.text(2,-1.8, sky+' sky', ha = 'center', fontsize = 'large')
    ax.axhline(y=0, xmin = 0, xmax = 1, color = 'darkgrey', linestyle = ':')
    ax.axvline(x=0.495, ymin=-8, ymax=1, color='darkgrey', linestyle='--')
    ax.axvline(x=1.49, ymin=-8, ymax=1, color='darkgrey', linestyle='--')
    ax.set_ylabel(u'$\Delta$CRE / W m$^{-2}$')
    plt.tight_layout()
    plt.savefig(plot_dir + 'su_forcing_comparison_bar_chart_varing_G_' + sky + '_sky_nudge_lev1',\
    ...
    # net_circ_adjust = np.round( forcings[0,0,0] - forcings [13,0,0] ,adjust_rounding)
    # net_circ_adjust_error = np.round( forcings[0,0,1] + forcings [13,0,1] ,uncertainty_rounding)
    # sw_circ_adjust = np.round( forcings[0,1,0] - forcings [13,1,0] ,adjust_rounding)
    # sw_circ_adjust_error = np.round( forcings[0,1,1] + forcings [13,1,1] ,uncertainty_rounding)
    # lw_circ_adjust = np.round( forcings[0,2,0] - forcings [13,2,0] ,adjust_rounding)
    # lw_circ_adjust_error = np.round( forcings[0,2,1] + forcings [13,2,1] ,uncertainty_rounding)
    
    
    # # T adjustment = uv nudged - uvtheta nudged 
    # net_T_adjust = np.round( forcings[13,0,0] - forcings [6,0,0] ,adjust_rounding)
    # net_T_adjust_error = np.round( forcings[13,0,1] + forcings [6,0,1] ,uncertainty_rounding)
    # sw_T_adjust = np.round( forcings[13,1,0] - forcings [6,1,0] ,adjust_rounding)
    # sw_T_adjust_error = np.round( forcings[13,1,1] + forcings [6,1,1] ,uncertainty_rounding)
    # lw_T_adjust = np.round( forcings[13,2,0] - forcings [6,2,0] ,adjust_rounding)
    # lw_T_adjust_error = np.round( forcings[13,2,1] + forcings [6,2,1] ,uncertainty_rounding)
