...
#!! need function for processing fluxes on levels??? !!#
...
def toa_flux(pp_file, sky_type, spinup_years = 0):
    """
    """
    ...
    if sky_type == 'all':
        ...
        diag_names = ['toa_incoming_shortwave_flux', \
                      'm01s01i519', \
                      'm01s02i519']
        #!! need to find out stash name for clear_clean
    ...
    # Extract just years after spin up of total down 3D cubes
    total_sw_down_spun = total_sw_down[spinup_years:]
    total_lw_down_spun = total_lw_down[spinup_years:]
    net_down_spun = net_down[spinup_years:]
    
    # Determine multi year mean 2D cubes using just years after spinup
    ...
    # Extract from area mean 1D cube just years after spinup
    yr_area_means_total_sw_down_spun = yr_area_means_total_sw_down[spinup_years:]
    yr_area_means_total_lw_down_spun = yr_area_means_total_lw_down[spinup_years:] 
    yr_area_means_net_down_spun = yr_area_means_net_down[spinup_years:]
    
    # Determine multiannual 0D area mean cubes using just years after spinup
...
#    cubes = [total_sw_down, total_lw_down, net_down]
##    annual_area_means = np.zeros((3,1))
#    annual_area_means = []
#    multiannual_means = []
#    multiannual_area_mean = []
#    
#    for i in np.arange(len(cubes)):
#        annual_area_means.append(area_mean_cube(cubes[i]))
#        print(annual_area_means)
#        multiannual_means.append(time_mean_cube(cubes[i]))
#        print(multiannual_means)
#        multiannual_area_mean.append(time_mean_cube(annual_area_means[i]))
#        print(multiannual_area_mean)
#

#        annual_area_means[i] = area_mean_cube(cubes[i])
#        multiannual_mean[i,0], multiannual_mean[i,1], multiannual_mean[i,2] = \
#        time_mean_cube(cubes[i])
#        multiannual_area_mean[i,0], multiannual_area_mean[i,1], multiannual_area_mean[i,2] = \
#        time_mean_cube(annual_area_means[i])
    #!! need to test works, and improve code !##
