...
for i, sky in enumerate(sky_type):
    ...
    filenames = ['calc_forcing_br793_control_nudging_free_flux_'+sky+'_sky_minus_ce067_bc_nudging_free_flux_'+sky+'_sky_1yr_spinup.nc',
                 'calc_forcing_by937_control_nudging_uvt_flux_'+sky+'_sky_minus_cd563_bc_nudging_uvt_flux_'+sky+'_sky_1yr_spinup.nc',
                 'calc_forcing_cb108_control_nudging_uvt_flux_'+sky+'_sky_minus_ce303_bc_nudging_uvt_flux_'+sky+'_sky_1yr_spinup.nc',
                 'calc_forcing_bz531_control_nudging_uv_flux_'+sky+'_sky_minus_cb470_bc_nudging_uv_flux_'+sky+'_sky_1yr_spinup.nc',
                 'calc_forcing_cb349_control_nudging_uv_flux_'+sky+'_sky_minus_ce304_bc_nudging_uv_flux_'+sky+'_sky_1yr_spinup.nc',
                 ]
...
#        sky_type = file[index2 + 7 : index1 - 5]
    ...
    # ax.bar(label_points - 6*width/2, forcings[0,:,0], width, yerr = forcings[0,:,1], color = 'black', label = 'free, n=34')
    
    # ax.bar(label_points - 3*width/2, forcings[1,:,0], width, yerr = forcings[1,:,1], color = 'red', label = 'uv' + char.theta() + ', G=1/6, n=4')
   
    # ax.bar(label_points - 0*width/2, forcings[2,:,0], width, yerr = forcings[2,:,1], color = 'peru', label = 'uv' + char.theta() + ', G=1/6, bl=1, ramp=1, n=4')
   
    # ax.bar(label_points + 3*width/2, forcings[3,:,0], width, yerr = forcings[3,:,1], color = 'royalblue', label = 'uv, G=1/6, n=4')

    # ax.bar(label_points + 6*width/2, forcings[4,:,0], width, yerr = forcings[4,:,1], color = 'indigo', label = 'uv, G=1/6, bl=1, ramp=1, n=4')
    ...
    # plt.tight_layout()
    # plt.savefig(plot_dir + 'bc_forcing_comparison_bar_chart_varing_G_' + sky + '_sky', dpi = 400)
    # plt.show()
    ...
    net_circ_adjust = np.round( forcings[0,0,0] - forcings [4,0,0] ,adjust_rounding)
    net_circ_adjust_error = np.round( forcings[0,0,1] + forcings [4,0,1] ,uncertainty_rounding)
    sw_circ_adjust = np.round( forcings[0,1,0] - forcings [4,1,0] ,adjust_rounding)
    sw_circ_adjust_error = np.round( forcings[0,1,1] + forcings [4,1,1] ,uncertainty_rounding)
    lw_circ_adjust = np.round( forcings[0,2,0] - forcings [4,2,0] ,adjust_rounding)
    lw_circ_adjust_error = np.round( forcings[0,2,1] + forcings [4,2,1] ,uncertainty_rounding)
    
    
    # T adjustment = uv nudged - uvtheta nudged 
    net_T_adjust = np.round( forcings[4,0,0] - forcings [2,0,0] ,adjust_rounding)
    net_T_adjust_error = np.round( forcings[4,0,1] + forcings [2,0,1] ,uncertainty_rounding)
    sw_T_adjust = np.round( forcings[4,1,0] - forcings [2,1,0] ,adjust_rounding)
    sw_T_adjust_error = np.round( forcings[4,1,1] + forcings [2,1,1] ,uncertainty_rounding)
    lw_T_adjust = np.round( forcings[4,2,0] - forcings [2,2,0] ,adjust_rounding)
    lw_T_adjust_error = np.round( forcings[4,2,1] + forcings [2,2,1] ,uncertainty_rounding)
    ...
    ax.text(2.35,0.75, 'BC: '+sky+' sky', ha = 'right', fontsize = 'large')
    ax.axhline(y=0, xmin = 0, xmax = 1, color = 'darkgrey', linestyle = ':')
    ax.set_ylabel(u'Radiative forcing / W m$^{-2}$')
    plt.tight_layout()
    plt.savefig(plot_dir + 'bc_forcing_erf_T_circ_adjust_bar_chart_' + sky + '_sky', dpi = 300, format = 'png')
    plt.show()