...
#    ax.bar(label_points - 11*width/2, flux_error[0,0,0], width, yerr = flux_error[0,0,1], color = 'black', label = 'free, n=34')
    ...
    ax.bar(label_points + 1*width/2, flux_error[8,:,0], width, yerr = flux_error[8,:,1], color = 'midnightblue', label = 'uv, G=1/24, n=4')
    ax.bar(label_points + 3*width/2, flux_error[9,:,0], width, yerr = flux_error[9,:,1], color = 'mediumblue', label = 'uv, G=1/12, n=4')
    ax.bar(label_points + 5*width/2, flux_error[10,:,0], width, yerr = flux_error[10,:,1], color = 'royalblue', label = 'uv, G=1/6, n=4')
    ax.bar(label_points + 7*width/2, flux_error[11,:,0], width, yerr = flux_error[11,:,1], color = 'cornflowerblue', label = 'uv, G=1/3, n=4')
    ax.bar(label_points + 9*width/2, flux_error[12,:,0], width, yerr = flux_error[12,:,1], color = 'lightsteelblue', label = 'uv, G=1/1, n=4')
    
    ax.bar(label_points + 12*width/2, flux_error[13,:,0], width, yerr = flux_error[13,:,1], color = 'indigo', label = 'uv, G=1/6, bl=1, ramp=1, n=4')
    ax.bar(label_points + 14*width/2, flux_error[14,:,0], width, yerr = flux_error[14,:,1], color = 'darkviolet', label = 'uv, G=1/6, bl=1, ramp=4, n=4')
    ...
    ax.text(-0.3,-1.0, sky+' sky', ha = 'left', fontsize = 'large')
    ax.axhline(y=0, xmin = 0, xmax = 1, color = 'darkgrey', linestyle = ':')
    ax.set_ylabel(u'TOA Flux error / W m$^{-2}$')
    plt.tight_layout()
    # plt.savefig(plot_dir + 'control_flux_error_comparison_varying_G_' + sky + '_sky', dpi = 400)
    plt.show()
