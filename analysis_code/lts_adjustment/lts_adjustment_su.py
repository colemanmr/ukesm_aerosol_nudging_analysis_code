...
current_map = plt.gca()
current_map.coastlines(linewidth = 1)
# plt.title('Free SU LTS adjustment')
plt.savefig(plot_dir + 'lts_adjustment_free_su_br793-bv046', dpi = 300, bbox_inches='tight')
plt.show()
...
for i in range(n_suites):
    ...
    current_map = plt.gca()
    current_map.coastlines(linewidth = 1)
    # plt.title('Nudged '+ titles[i] + ' SU LTS adjustment')
    plt.savefig(plot_dir + 'lts_adjustment_nudged_su_' + nudged_cont_names[i]\
