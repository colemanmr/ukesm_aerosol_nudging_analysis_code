...
for i in range(no_suites):
    ...
    ait_ins_cube = iris.load_cube(diag_dir + 'aer_burden_mode_nmbr_ratios_insol_ait_'\
                      + suite_names[i] + '_time_mean.pp')
...
modes = (nuc, ait, acc, coa, ait_ins)
mode_adjs = (nuc_adj, ait_adj, acc_adj, coa_adj, ait_ins_adj)
...
# list of mode names
mode_names = ['nuc', 'ait', 'acc', 'coa', 'ait_ins']
...
for i in range(no_nudging_types):
    ...
    for j in range(no_modes):
        ...
        plt.savefig(plot_dir + mode_names[j] + 'mode_nmbr_conc_lev11_adjustment_' +\
            suite_names[i*2] + '-' + suite_names[i*2+1], dpi = 300)
        plt.show()
        ...
        figure = plt.gca()
        figure.coastlines(linewidth = 1)
        plt.savefig(plot_dir + mode_names[j] + 'mode_nmbr_conc_lev11_control_' +\
            suite_names[i*2], dpi = 300)
        plt.show()    
