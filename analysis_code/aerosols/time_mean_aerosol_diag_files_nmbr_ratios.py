...
for i in range(no_suites):
    ...
    aer_modes = iris.load(diag_dir + 'aer_burden_mode_nmbr_ratios_sol_' + suites[i] + '.pp')
    
    # check diags loaded in expected order
    print(aer_modes)
    ...
    iris.save(cubes, diag_dir + 'aer_burden_mode_nmbr_ratios_sol_' + suites[i] + \
              '_time_mean.pp')
