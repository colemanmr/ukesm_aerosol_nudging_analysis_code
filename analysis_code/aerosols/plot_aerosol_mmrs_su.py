...
### For each suite calculate total burden and ait+acc+coarse burden
...
adj_intervals = np.linspace(start = - 0.1* 1e-8, stop = 1.4e-8, num = 16)
cont_intervals = np.linspace(start = 0, stop = 1.8e-8, num = 10)
...
for i in range (no_nudging_types):
    ...
    qplt.contourf(cont[11], \
                  ...
                  cmap = 'magma')
    plt.title('all mode SU burden 1km in control ' + \
              suite_names[i*3])
    figure = plt.gca()
    figure.coastlines(linewidth = 1)
