...
filename = 'by762_6hourly_nudge_diags_ap7.pp'
...
plt.savefig(plot_directory + '/fixed_by762_N48_time_step_' + str(timestep * 18) + '_nudging_diag_profile_analysis_lat0lon0',\
            dpi = 300)
plt.show()
...
ts_after_nudge = after_nudge[timestep,:,0,0]
ts_raw_input = raw_input[timestep,:,0,0]
ts_anal = nudge_analysis[timestep,:,0,0]
...
plt.legend()
plt.tight_layout()
plt.savefig(plot_directory + '/fixed_by762_N48_time_step_' + str(timestep * 18) + '_nudging_diag_profile_differences_lat0lon0',\
            dpi = 300)
plt.show()
