...
free_cont_file = diag_dir + 'theta_month_mean_br793_2014-2023.pp'
free_cont_10years = iris.load(free_cont_file)
free_cont = free_cont_10years[0][0:12]
print(free_cont)
...
for suite in cont_suite_names:
    if suite == 'cb108':
            cont_theta_5years = iris.load_cube(diag_dir + 'theta_month_mean_' \
                           + suite + '_2014-2018.pp')
            cont_theta = cont_theta_5years[0:12]
    else:
...
# initialise list for holding monthly level mean theta cont_errors
mon_lev_mean_theta_cont_err = []

# calculate cont_errors and level mean for each suite's monthly mean
for i in range(len(cont_suite_names)):
    monthly_theta_cont_error = free_cont - conts_theta[i] 
    
    level_mean_theta_cont_error = \
        flux_mod.area_mean_cube(monthly_theta_cont_error)
        
    mon_lev_mean_theta_cont_err.append(level_mean_theta_cont_error)   
...
# nudged_cont_error_monthly = nudged_cont_monthly - nudged_pert_monthly
...
# surf_t_diag_dir = file_loc.diag_dir + 'other_cont_errors/surf_t/'
...
# nudged_cont_error_monthly_surf_t = nudged_cont_monthly_surf_t[0] - nudged_pert_monthly_surf_t[0]
# area_mean_nudged_cont_error_monthly_surf_t = flux_mod.area_mean_cube(nudged_cont_error_monthly_surf_t)
...
# area_mean_nudged_cont_error_monthly_surf_t_array = area_mean_nudged_cont_error_monthly_surf_t.data
...
cb108_months = np.linspace(0.5,12.5,num=12)

save_target = new_plot_dir + \
    'month_lev_mean_theta_cont_error_time_series_varying_time_res'
...
for i in range(len(cont_suite_names)):
    ...
    #                  mon_lev_mean_theta_cont_err[i][:,index_levels[j]].data,\
...
plt.xlabel('Months')
plt.ylabel('Potential temperature control error / K ')
plt.xlim(0,13)
plt.ylim(-0.28, 0.14)
# plt.ylim(-4.5,3.5)
# plt.xticks([0,12,24,36,48,60])
# plt.title(r'Monthly level mean $\theta$ cont_error, ' + r'uv$\theta$ G=1/6 BL, 3hr input')
# plt.title(r'Monthly level mean $\theta$ cont_error, ' + 'free SU')
# plt.title(r'Monthly level mean $\theta$ cont_error G = 1/6, BL, 3hr input')
plt.savefig(save_target, dpi = 300, bbox_inches='tight')
plt.show()



# # plot level mean cont_errors with monthly and total time means
# #!! not full height! !!#
# month = 5
# index_month = month - 1

# plt.figure()
# iplt.plot(level_mean_free_cont_error[0:63], \
#           level_mean_free_cont_error[0:63].coord('level_height'),
#           label = 'free',
#           color = 'black')
# iplt.plot(level_mean_nudged_cont_error[0:63],
#           level_mean_nudged_cont_error[0:63].coord('level_height'),
#           label = 'uvt G = 1/6 BL',
#           color = 'red')
# iplt.plot(level_mean_nudged_cont_error_monthly[index_month,0:63],
#           level_mean_nudged_cont_error_monthly[index_month,0:63].coord('level_height'),
...
# plt.xlabel('Potential temperature cont_error / K ')
# plt.title(r'SU perturbed $\theta$ cont_error profile, month ' + str(month))
# # plt.savefig(new_plot_dir + 'level_mean_theta_su_cont_errors_free_and_cb108-cb109_with_month_mean_' + str(month), dpi=300)
# plt.show()