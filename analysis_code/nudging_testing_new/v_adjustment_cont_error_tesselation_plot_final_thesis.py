...
free_cont_time_mean_4years = iris.load_cube(free_cont_file_4years, 'mean_v_wind')
...
free_cont_time_mean = iris.load_cube(free_cont_file, 'mean_v_wind')
# free_cont_time_mean = free_cont[1]

free_pert_su_file = diag_dir + 'time_mean_uv_bv046.nc'
free_pert_su_time_mean = iris.load_cube(free_pert_su_file, 'mean_v_wind')
...
for i in range(n_suites):
    ...
    nudged_cont = iris.load_cube(nudged_cont_file, 'mean_v_wind')
    nudged_cont_time_mean = nudged_cont
    
    nudged_pert = iris.load_cube(nudged_pert_file, 'mean_v_wind')
    nudged_pert_time_mean = nudged_pert
...
fig.colorbar(mesh, cax=cbar_ax, label = r'v-wind adjustment / m s$^{-1}$',
             orientation = 'horizontal')

plt.savefig(plot_dir + 
            'nudging_v_adjustment_levels_zonal_free_uv_uvt_tesselation',
...
fig.colorbar(mesh, cax=cbar_ax, label = r'v-wind control error / m s$^{-1}$',
             orientation = 'horizontal')

plt.savefig(plot_dir + 
            'nudging_v_cont_error_levels_zonal_free_uv_uvt_tesselation',
            dpi = 300, bbox_inches='tight')
plt.show()
